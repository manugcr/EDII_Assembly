...
;   la senal es 700ms baja 4300ms alta. Si el toggle esta desactivado es una 
;   senal alta constante. 
;
;   Fosc = 2MHz.
; ******************************************************************************
...
; ***************************** REG CONFIG *************************************  
SETUP	    MOVLW   .61		    ; TMR0 en 60 para llegar a 100[ms]
	    MOVWF   TMR0_VAL
	    MOVLW   .8		    ; Cuento 7 veces 100 para llegar a 700[ms]
	    MOVWF   CNT_LOW
	    MOVLW   .44		    ; Cuento 43 veces 100 para llegar a 4.3[s]
	    MOVWF   CNT_HIGH	    
	    ...
	    BSF	    PORTB, RB4	    ; Arranco largando un 1 por el pin RB2
	    CALL    LOAD_TMR0	    ; El timer comienza a contar los 100[ms]
	    GOTO    MAIN
...
; *********************** SUB-ROUTINES *****************************************
LOAD_TMR0   MOVF    TMR0_VAL, W	    ; Cargo TMR0 con 60 para contar 100[ms]
	    MOVWF   TMR0
	    RETURN
	    
LOAD_LOW    MOVLW   .8		    ; Cuento 7 veces 100 para llegar a 700[ms]
	    MOVWF   CNT_LOW
	    RETURN

LOAD_HIGH   MOVLW   .43		    ; Cuento 43veces 100 para llegar a 4300[ms]
	    MOVWF   CNT_HIGH
	    RETURN
	 
SET_RBFLAG  COMF    RB_FLAG
	    BSF	    PORTB, RB4	    ; Por default el pin RB4 esta prendido.
	    GOTO    END_ISR
